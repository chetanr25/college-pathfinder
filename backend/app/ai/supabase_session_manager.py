...
class ChatSession:
    ...
    def add_message(self, message: Message):
        ...
        if message.role == "user" and not self.preview:
            ...
            self.title = message.content[:40] + ("..." if len(message.content) > 40 else "")
...
class SupabaseSessionManager:
    ...
    def __init__(self):
        ...
        if not supabase_url or not supabase_key:
            print("⚠️ WARNING: Supabase credentials not configured. Using in-memory storage.")
    ...
    def create_session(self, user_id: str = None, title: str = "New Chat") -> ChatSession:
    ...
    def delete_session(self, session_id: str, user_id: str = None) -> bool:
        ...
        if self.supabase:
            try:
                ...
                # Delete session
                query = self.supabase.table('chat_sessions').delete().eq('id', session_id)
                if user_id:
                    query = query.eq('user_id', user_id)
                query.execute()
                return True
    ...
    def add_message(self, session_id: str, message: Message, user_id: str = None) -> bool:
    ...
    def get_messages(self, session_id: str, limit: Optional[int] = None, user_id: str = None) -> List[Message]:
    ...
    def list_sessions(self, user_id: str = None) -> List[Dict[str, Any]]:
        ...
        try:
            result = self.supabase.table('chat_sessions').select('*').eq('user_id', user_id).order('updated_at', desc=True).execute()
            
            if result.data:
                sessions = []
                for s in result.data:
                    # Get message count
                    count_result = self.supabase.table('chat_messages').select('*', count='exact').eq('session_id', s['id']).execute()
    ...
    def save_message_async(self, session_id: str, message: Message, user_id: str = None):
