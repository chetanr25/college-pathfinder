...
class ResponseTemplates:
    ...
    @staticmethod
    def colleges_by_rank(colleges: List[Dict], rank: int, round_num: int, limit: int, branches: List[str] = None) -> str:
        if not colleges:
        ...
        for i, college in enumerate(colleges[:display_limit], 1):
            ...
            lines.append(f"| {i} | {name} | {branch} | {ResponseTemplates.format_number(cutoff)} | {chance} |")
    ...
    @staticmethod
    def colleges_by_branch(colleges: List[Dict], branch: str, round_num: int, limit: int) -> str:
        if not colleges:
        ...
        for i, college in enumerate(colleges[:display_limit], 1):
            ...
            if len(name) > 45:
                name = name[:42] + "..."
            cutoff = college.get('cutoff_rank', 0)
            lines.append(f"| {i} | {name} | {ResponseTemplates.format_number(cutoff)} |")
    ...
    def college_info(info: Dict) -> str:
        ...
        for branch in branches:
            ...
            r1 = ResponseTemplates.format_number(cutoffs.get('round1')) if cutoffs.get('round1') else '-'
            r2 = ResponseTemplates.format_number(cutoffs.get('round2')) if cutoffs.get('round2') else '-'
            r3 = ResponseTemplates.format_number(cutoffs.get('round3')) if cutoffs.get('round3') else '-'
    ...
    def compare_colleges(comparison: Dict) -> str:
        ...
        lines.append("| Code | " + " | ".join([c.get('college_code', '-') for c in colleges]) + " |")
        lines.append("| Branches | " + " | ".join([str(c.get('total_branches', 0)) for c in colleges]) + " |")
        lines.append("| Best Cutoff | " + " | ".join([ResponseTemplates.format_number(c.get('best_cutoff', 0)) for c in colleges]) + " |")
        lines.append("| Avg Cutoff | " + " | ".join([ResponseTemplates.format_number(c.get('avg_cutoff', 0)) for c in colleges]) + " |")
        lines.append("| Worst Cutoff | " + " | ".join([ResponseTemplates.format_number(c.get('worst_cutoff', 0)) for c in colleges]) + " |")
    ...
    def analyze_rank(analysis: Dict) -> str:
        ...
        if categories.get('best'):
            ...
            for c in categories['best'][:5]:
                ...
                if len(branch) > 20:
                    branch = branch[:17] + "..."
                lines.append(f"| {name} | {branch} | {ResponseTemplates.format_number(c.get('cutoff_rank', 0))} |")
            lines.append("")
    ...
    def branch_popularity_specific(data: Dict) -> str:
        ...
        if top_colleges:
            ...
            for c in top_colleges[:10]:
                ...
                if len(name) > 40:
                    name = name[:37] + "..."
                lines.append(f"| {name} | {ResponseTemplates.format_number(c.get('cutoff_rank', 0))} |")
    ...
    async def route(self, message: str, session=None) -> RouterResult:
        try:
            ...
            if extracted.get("needs_gemini_response", True):
                return RouterResult(handled=False, intent=intent, needs_gemini=True, params=extracted)
            
            response = await self._execute_intent(extracted)
            if response:
                return RouterResult(handled=True, response=response, intent=intent, params=extracted)
    ...
    async def _execute_intent(self, params: Dict) -> Optional[str]:
        ...
        try:
            if intent == "colleges_by_rank":
                if not rank:
                    return None
                colleges = CollegeService.get_colleges_by_rank(rank=rank, round=round_num, limit=limit)
                ...
                if not resolved:
                    return None
                colleges = CollegeService.get_colleges_by_branch(branch=resolved[0], round=round_num, limit=limit)
                return self.templates.colleges_by_branch(colleges, resolved[0], round_num, limit)
            
            elif intent == "colleges_by_rank_and_branch":
                if not rank:
                    return None
                resolved = intent_extractor.resolve_branches(branch_terms) if branch_terms else None
                if resolved:
                    ...
                    return self.templates.colleges_by_rank(colleges, rank, round_num, limit, resolved)
                else:
                    colleges = CollegeService.get_colleges_by_rank(rank=rank, round=round_num, limit=limit)
                    return self.templates.colleges_by_rank(colleges, rank, round_num, limit)
                ...
                if not codes:
                    results = CollegeService.search_college_by_name(college_names[0], limit=5)
                ...
                if branch_terms:
                    resolved = intent_extractor.resolve_branches(branch_terms)
                    if resolved:
                        data = CollegeService.get_branch_popularity(branch_name=resolved[0], round=round_num)
                        return self.templates.branch_popularity_specific(data)
                data = CollegeService.get_branch_popularity(branch_name=None, round=round_num)
