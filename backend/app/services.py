...
class CollegeService:
    ...
    def get_colleges_by_rank(
        rank: int, round: int = 1, limit: Optional[int] = 10, sort_order: str = "asc"
    ) -> List[Dict]:
        """
    ...
    def get_cutoff_trends(college_code: str, branch: str) -> Dict:
        ...
        return {
            ...
            "cutoff_trends": {
                ...
                if result["GM_rank_r1"] is not None
                else 0,
                "round2": result["GM_rank_r2"]
                if result["GM_rank_r2"] is not None
                else 0,
                "round3": result["GM_rank_r3"]
                if result["GM_rank_r3"] is not None
                else 0,
            },
        }
    ...
    def search_college_by_name(query: str, limit: int = 10) -> List[Dict]:
        ...
        for college in all_colleges:
            ...
            if len(query_lower) <= 5:
                ...
                if (
                    initials.lower().startswith(query_clean)
                    or query_clean == initials.lower()
                ):
    ...
    def match_branch_names(query: str, limit: int = 10) -> List[Dict]:
        ...
        for branch in all_branches:
            ...
            if max_score > 0.4:  # Minimum threshold
                matches.append(
                    {"branch_name": branch_name, "match_score": round(max_score, 3)}
                )
    ...
    def analyze_rank_prospects(rank: int, round: int = 1) -> Dict:
        ...
        results = execute_query(
            query, (round, round, round, round, rank, round, rank, round, rank)
        )
    ...
    def get_branch_popularity(branch_name: str = None, round: int = 1) -> Dict:
        ...
        if branch_name:
            ...
            """
            results = execute_query(
                query, (round, round, round, branch_name, round, round, round)
            )
            ...
            """
            results = execute_query(
                query, (round, round, round, round, round, round, round, round, round)
            )
                ...
                branches_summary.append(
                    {
                        ...
                        "best_cutoff": builtin_round(row["best_cutoff"])
                        if row["best_cutoff"]
                        else 0,
                        "avg_cutoff": builtin_round(row["avg_cutoff"])
                        if row["avg_cutoff"]
                        else 0,
                        "competitiveness": "High"
                        if row["best_cutoff"] and row["best_cutoff"] < 5000
                        else "Medium"
                        if row["best_cutoff"] and row["best_cutoff"] < 20000
                        else "Low",
                    }
                )
