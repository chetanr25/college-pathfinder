...
from fastapi import APIRouter, WebSocket, WebSocketDisconnect, HTTPException, Query, Request, Header
    ...
    async def generate_sse() -> AsyncGenerator[str, None]:
        ...
        async def process_ai_response():
            ...
            try:
                ...
                # Add user message to session
                user_msg = Message(role="user", content=user_message, session_id=actual_session_id)
                session.add_message(user_msg)
                ...
                # Add assistant response to session
                assistant_msg = Message(role="assistant", content=response_text, session_id=actual_session_id)
                session.add_message(assistant_msg)
                assistant_msg_holder["msg"] = assistant_msg
                
                # Save assistant message to Supabase
                if user_id and not is_temp_session:
                    session_manager.add_message(actual_session_id, assistant_msg, user_id)
