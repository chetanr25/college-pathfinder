...
async def send_prediction_summary(
    request: PredictionEmailRequest, background_tasks: BackgroundTasks
):
    ...
    subject = (
        f"🎓 Your KCET College Predictions - {len(request.colleges)} Matches Found!"
    )
...
async def send_welcome(request: EmailRequest, background_tasks: BackgroundTasks):
    ...
    return {"success": True, "message": f"Welcome email queued for {request.email}"}


@router.post("/send-comparison")
async def send_comparison(
    request: ComparisonEmailRequest, background_tasks: BackgroundTasks
):
    ...
    return {"success": True, "message": f"Comparison report queued for {request.email}"}


@router.post("/send-counseling-reminder")
async def send_counseling_reminder(
    request: ReminderEmailRequest, background_tasks: BackgroundTasks
):
...
async def send_shortlist(
    request: ShortlistEmailRequest, background_tasks: BackgroundTasks
):
    ...
    return {"success": True, "message": f"Shortlist summary queued for {request.email}"}


@router.post("/send-admission-tips")
async def send_admission_tips(request: EmailRequest, background_tasks: BackgroundTasks):
    """Send admission tips"""
    ...
    return {"success": True, "message": f"Admission tips queued for {request.email}"}
...
async def send_success_stories(
    ...
    return {"success": True, "message": f"Success stories queued for {request.email}"}


@router.post("/send-bulk")
async def send_bulk_emails(
    request: BulkEmailRequest, background_tasks: BackgroundTasks
):
    """Send bulk emails"""
