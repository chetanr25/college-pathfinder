...
def send_detailed_report(user_email: str, report_data: dict):
    ...
    context = {
        ...
        "high_chance_count": sum(
            1
            for c in report_data.get("colleges", [])
            if c.get("admission_chance") == "High"
        ),
        "branches_count": len(
            set(c.get("branch") for c in report_data.get("colleges", []))
        ),
...
def send_cutoff_trends(user_email: str, trends_data: dict):
    ...
    context = {
        ...
        "cutoff_year_3": trends_data.get("cutoff_2025"),
        "trend_year_2": trends_data.get("trend_2023", "trend-up"),
        "trend_year_3": trends_data.get("trend_2025", "trend-down"),
