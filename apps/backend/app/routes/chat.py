"""
Chat routes for AI counselor with PostgreSQL integration

Includes SSE streaming endpoint for real-time chat and REST endpoints for session management.
All sessions are stored in PostgreSQL with app-level user access control.
    ...
    try:
        while True:
            ...
            if msg_type == "chat_message" or (
                ...
                # Create real session in DB if this is first message and we have a user
                if is_temp_session and user_id:
                    real_session = await session_manager.create_session(
                ...
                if user_id and not is_temp_session:
                    await session_manager.add_message(session_id, user_msg, user_id)
                ...
                try:
                    ...
                    if user_id and not is_temp_session:
                        await session_manager.add_message(session_id, assistant_msg, user_id)
    ...
    async def generate_sse() -> AsyncGenerator[str, None]:
        ...
        async def process_ai_response():
            ...
            try:
                ...
                # Save to DB
                if user_id and not is_temp_session:
                    await session_manager.add_message(actual_session_id, user_msg, user_id)
...
async def create_session(user_id: str = None, title: str = "New Chat"):
    ...
    session = await session_manager.create_session(user_id, title)
...
async def get_messages(session_id: str, limit: int = None, user_id: str = None):
    ...
    messages = await session_manager.get_messages(session_id, limit, user_id)
